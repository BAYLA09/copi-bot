...
def role_prefix(role: str) -> str:
    ...
    raise ValueError(f"Unsupported OAuth role: {role}")


def default_api_host(role: str) -> str:
    if role == "destination":
        return DEFAULT_DEST_API_HOST
...
def _env(name: str, *, role: str = "source") -> str:
    prefix = role_prefix(role)
    value = os.getenv(f"{prefix}{name}", "").strip()
    if value:
        return value
    if role == "source":
        return os.getenv(f"{DEST_PREFIX}{name}", "").strip()
    return ""
...
def tokens_still_valid(*, role: str = "source") -> bool:
    access_token = _env("ACCESS_TOKEN", role=role)
    expires_raw = _env("TOKEN_EXPIRES_AT", role=role)
    ...
    *,
    ...
    upsert_env_values(
        ENV_FILE,
        {
            ...
            f"{prefix}ACCESS_TOKEN": str(token_payload["accessToken"]),
            f"{prefix}REFRESH_TOKEN": str(token_payload["refreshToken"]),
            f"{prefix}TOKEN_EXPIRES_AT": str(int(expires_at)),
            f"{prefix}TRADER_LOGIN": str(trader_login),
            f"{prefix}API_HOST": api_host,
            f"{prefix}REDIRECT_URI": redirect_uri,
            f"{prefix}OAUTH_SCOPE": scope,
        },
    )


def preferred_trader_login(*, role: str = "source") -> int | None:
    raw = _env("TRADER_LOGIN", role=role)
    if raw.isdigit():
        return int(raw)
    ...
    default_source = os.getenv("CTRADER_SOURCE_TRADER_LOGIN", "9093520").strip()
    if default_source.isdigit():
        return int(default_source)
...
def ensure_fresh_tokens(*, role: str = "source", force_refresh: bool = False) -> None:
    """Refresh access tokens in .env when expired, using the official endpoint."""
    if tokens_still_valid(role=role) and not force_refresh:
        return

    client_id = _env("CLIENT_ID", role=role)
    client_secret = _env("CLIENT_SECRET", role=role)
    refresh_token = _env("REFRESH_TOKEN", role=role)
    trader_login = _env("TRADER_LOGIN", role=role)
    api_host = _env("API_HOST", role=role) or default_api_host(role)
    redirect_uri = _env("REDIRECT_URI", role=role) or DEFAULT_REDIRECT_URI
    scope = _env("OAUTH_SCOPE", role=role) or DEFAULT_SCOPE

    if not all([client_id, client_secret, refresh_token, trader_login]):
        role_hint = "login.py" if role == "source" else "login.py --role destination"
        raise SystemExit(
            f"OAuth tokens are missing or expired for {role}. Run `{role_hint}` first."
        )

    logger.info(
        "Access token expired or near expiry for %s. Refreshing via official endpoint...",
        role,
    )
    ...
    logger.info("Refreshed OAuth tokens for %s saved to %s", role, ENV_FILE)
...
def run_login(
    *,
    ...
    if tokens_still_valid(role=role) and not force:
        logger.info(
            "Valid OAuth tokens for %s already exist in %s. Skipping browser login.",
            role,
            ENV_FILE,
        )
        if role == "destination":
            logger.info("Run `python copier.py` or use --force to login again.")
        else:
            logger.info("Run `python monitor_api.py` or use --force to login again.")
        return
    ...
    if not client_id or not client_secret:
        prefix = role_prefix(role)
        raise SystemExit(
            f"Set {prefix}CLIENT_ID and {prefix}CLIENT_SECRET in .env before running login.py."
        )

    redirect_uri = _env("REDIRECT_URI", role=role) or DEFAULT_REDIRECT_URI
    scope = _env("OAUTH_SCOPE", role=role) or DEFAULT_SCOPE
    api_host = _env("API_HOST", role=role) or default_api_host(role)
    preferred_login = preferred_trader_login(role=role)
    ...
    logger.info("Saved OAuth tokens for %s to %s", role, ENV_FILE)
    logger.info("Trader login: %s", trader_login)
    if role == "destination":
        logger.info("You can now run: python copier.py")
    else:
        logger.info("You can now run: python monitor_api.py")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="source",
        help="Save OAuth tokens for the source monitor or destination copier.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
